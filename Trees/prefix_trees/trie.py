...
class trie(object):
    ...
    def print_(
        self, start_node
    ):  # simply printing all the letters in the vocab for now
        for child in start_node.children:
            self.print_(child)
        if start_node.val != None:
            print(start_node.val)

    def print_vocab(
        self, node, first=False, word=""
    ):  # printing all the words in the vocabulary
